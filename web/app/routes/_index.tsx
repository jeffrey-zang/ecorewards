...
export const meta: MetaFunction = () => {
  return [{ title: "EcoRewards" }, { name: "description", content: "Earn rewards by eco-friendly actions" }];
};

export async function loader() {
  const res = await fetch("http://localhost:3000/api/v1/rewards/getAll",
    {
      method: "GET",
      headers: {
    ...
    <div className="flex flex-col items-center justify-center">
      ...
      <div className="flex flex-col w-full mt-4 h-screen">
        <div className="rounded-lg w-full p-8" style={{
          ...
          <button className="bg-green-600 text-white px-4 py-2 rounded-lg mt-4 hover:opacity-90 transition-opacity" onClick={() => {document.getElementById('rewards')?.scrollIntoView()}}>Learn More &rarr;</button>
        </div>
        <div className="flex flex-col w-full mt-4" id="rewards" ref={animationParent}>
          {Object.entries(categorizedRewards).map(([category, rewards]: any) => (
...
function SearchBar(props: React.HTMLAttributes<HTMLElement>) {
  ...
  const handleFormSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    console.log(query === "")
    if (query.trim() === "" || query === null || query === undefined || query === " " || query === "") {
      navigate("/");
      return;
    }
